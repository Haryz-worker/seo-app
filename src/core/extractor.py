# extractor.py — convert HTML bytes to structured JSON (with real anchors)
...
from .utils import (
    to_unicode, clean_space, is_http, norm_url, host_key, dedup,
    detect_lang_fallback, count_words, ensure_dir, save_json
)

SOCIAL_HOSTS = {
    "facebook.com", "twitter.com", "x.com", "linkedin.com",
    "pinterest.com", "wa.me", "api.whatsapp.com", "t.me"
}
SKIP_SCHEMES = ("javascript:", "mailto:", "tel:", "#")
SKIP_PATTERNS = ("/share", "/print", "/wp-json", "/feed", "/amp", "/?replytocom=")
...
def _clean_anchor_text(txt: str) -> str:
    ...
    return t[:160]  # cap to avoid huge anchors


def _anchor_of(a_node) -> str:
    # prefer visible text; fallback to title attr if empty
...
def links_from_page_with_anchors(full_doc: LH.HtmlElement, base_url: Optional[str]) -> List[Dict]:
    ...
    skip_xpath = " | ".join([
        "//header//a[@href]", "//footer//a[@href]", "//nav//a[@href]", "//aside//a[@href]",
        "//*[@class='site-header']//a[@href]", "//*[@class='site-footer']//a[@href]",
        "//*[@class='menu']//a[@href]", "//*[@class='sidebar']//a[@href]",
        "//*[@class='breadcrumbs']//a[@href]", "//*[@class='share']//a[@href]",
        "//*[@class='social']//a[@href]", "//*[@class='tags']//a[@href]",
    ])
...
def _dedup_link_items(items: List[Dict]) -> List[Dict]:
    """Deduplicate by (url, anchor_norm)."""
    seen, out = set(), []
    for it in items:
        url = it.get("url")
...
def classify_link_items(items: List[Dict], base_url: Optional[str]) -> Dict:
    ...
    if not base_url:
        ...
        for it in items:
            try:
                (internal if host_key(it["url"]) == b else external).append(it)
            except Exception:
                external.append(it)
...
def html_bytes_to_json(
    ...
    # images (detailed in article, flat in top)
