...
def status_from_range(value: int, lo: int, hi: int) -> str:
    if value < lo: return "short"
    if value > hi: return "long"
    return "ok"
...
def _normalize_links(links_obj: Dict) -> Dict:
    """
    Accepts either:
      - NEW schema: {"counts": {...}, "internal_links":[{"url","anchor"}], "external_links":[{"url","anchor"}]}
      - LEGACY schema: {"internal":[...], "external":[...], "all":[...]}
    Returns unified:
      {
        "counts": {total, internal, external, internal_ratio, external_ratio},
        "internal_links": [{"url": ...}],   # anchor dropped in output by design
        "external_links": [{"url": ...}]
      }
    """
    if not isinstance(links_obj, dict):
        return {"counts": {"total": 0, "internal": 0, "external": 0, "internal_ratio": 0.0, "external_ratio": 0.0},
    ...
    if "internal_links" in links_obj or "external_links" in links_obj:
        ...
        internal_urls = [{"url": it.get("url")} for it in internal_items if it and it.get("url")]
        external_urls = [{"url": it.get("url")} for it in external_items if it and it.get("url")]
        if "counts" in links_obj and isinstance(links_obj["counts"], dict):
            counts = links_obj["counts"]
            total = counts.get("total")
            internal = counts.get("internal")
            external = counts.get("external")
            # recompute ratios safely if missing
            if total in (None, 0):
                total = len(internal_urls) + len(external_urls)
            if internal is None: internal = len(internal_urls)
            if external is None: external = len(external_urls)
        ...
        return {
            "counts": {
                ...
                "internal_ratio": internal_ratio, "external_ratio": external_ratio
            },
    ...
    return {
        ...
        "internal_links": [{"url": u} for u in internal_list],
        "external_links": [{"url": u} for u in external_list]
    }
...
def analyze_one(json_path: str, focus_terms: List[str], thresholds: Dict) -> Dict:
    ...
    # Links + warnings (normalize new/legacy)
    warnings = []
    ...
    # Focus keywords
    fk_stats = [keyword_stats(text, heads, page, h1, final_url, k) for k in (focus_terms or [])]

    # Prepare output links (drop anchors by design)
    def drop_anchor(items: List[Dict]) -> List[Dict]:
        return [{"url": it.get("url")} for it in (items or []) if it and it.get("url")]
    ...
    report = {
        ...
        "links": {
            ...
            "internal_links": drop_anchor(internal_links),
            "external_links": drop_anchor(external_links)
        },
        "keywords": fk_stats,
        "technical": {
...
def run(dir_path: str, input_file: str, thresholds: Dict, outfile: str = "report.json", pretty: bool = True) -> str:
    # pick the first JSON file in dir_path
    ...
    # load focus keywords from Input.json (urls, focuskeywords)
